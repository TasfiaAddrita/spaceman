...
def print_ascii(secret_word_len, num_of_guesses):
    ...
    ascii_guess = int((len(ascii_) - (len(ascii_) % secret_word_len)) / secret_word_len) # divides ascii into parts depending on number of letters in secret word

    if num_of_guesses != secret_word_len:
        lines_to_print = range(ascii_guess * num_of_guesses)
    else:
        lines_to_print = range(len(ascii_)) # if guess = 1, then print full ascii art
...
def spaceman(secret_word):
    ...
    user_letter_guesses = ['_' for i in range(len(secret_word))] # populates list that the user's input will go in with underscores
    alphabet = "abcdefghijklmnopqrstuvwxyz"
    num_of_guesses = 0
    continue_ = True
    while num_of_guesses < len(secret_word) and continue_ == True:
        letter = input('Enter a letter: ').lower().strip()
        # checks if letter is valid -- input is one letter, alpha char, and has not been used
        if len(letter) > 1:
            ...
            # if user's letter is in the secret word
...
def sinister(user_letter_guesses):
    secret_word_len = len(user_letter_guesses)
    words_list = list_words()
    words_list_len = len(list_words())
    half_index = 0

    while half_index != secret_word_len:
        if words_list_len % 2 == 0:
            pass
        else:
            half_index = words_list[math.floor(words_list_len / 2)]
'''
